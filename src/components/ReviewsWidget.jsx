  ...
  return (
    <>
      <button
        ...
        className="flex items-center gap-1.5 bg-pink-500 hover:bg-pink-600 transition-colors text-white px-3 py-1.5 rounded-lg shadow-md text-sm"
      >
      ...
      {/* Десктопная версия - выпадающая панель */}
      {isOpen && !isMobile && (
        <div className="absolute top-full right-0 mt-1 w-80 max-w-[calc(100vw-2rem)] bg-white rounded-lg shadow-xl border z-50 max-h-[80vh] overflow-hidden">
          <div className="p-4 max-h-[80vh] overflow-y-auto">
            ...
            {reviews.length === 0 ? (
              <p className="text-gray-500 text-sm text-center py-4">Пока нет отзывов</p>
            ) : (
              <div className="space-y-3">
                ...
                {reviews.length > 20 && (
                  <p className="text-xs text-gray-400 text-center">Показаны последние 20 отзывов</p>
                )}
              </div>
            )}
          </div>
